...
import apcsa_q4.project.Events.Hurt;
...
public class OBJ_Bomb extends SuperDropedItem {
   ...
   public OBJ_Bomb(int worldCol, int worldRow, boolean armed) {
      ...
      name = "Bomb";
      try {
         image = ImageIO.read(getClass().getResourceAsStream("/sprites/Objects/DroppedItem/bomb.png"));
      } catch (Exception e) {
         e.printStackTrace();
      }
      if (armed) {
   ...
   public BufferedImage hueShift(BufferedImage bi, int shift) {
      ...
      for (int Y = 0; Y < HEIGHT; Y++) {
         for (int X = 0; X < WIDTH; X++) {
            ...
            if (RGB != 0) {
               processed.setRGB(X, Y, Color
                     .getHSBColor(hue, HSV[1], HSV[2] + (maxCounter / (shift + 1)) / maxCounter + .0085f).getRGB());
            } else {
