package apcsa_q4.project.Entity;
...
import apcsa_q4.project.Events.*;
import apcsa_q4.project.Inventory.Inventory;
import apcsa_q4.project.Inventory.Item;
import apcsa_q4.project.Main.Consts;
import apcsa_q4.project.Main.GamePanel;
import apcsa_q4.project.Main.KeyHandler;
import apcsa_q4.project.Monster.Monster;
import apcsa_q4.project.Objects.*;
...
public class Player extends Entity {
    ...
    public void setDefaultValues() {
        ...
        solidArea = new Rectangle((int) screenX + 4 * Consts.SCALE, (int) screenY + 6 * Consts.SCALE, 8 * Consts.SCALE,
                10 * Consts.SCALE);
    ...
    public void getPlayerImage() {
        ...
        try {
            ...
            up0 = ImageIO.read(getClass().getResourceAsStream("/sprites/Player/up0.png"));
            ...
            down0 = ImageIO.read(getClass().getResourceAsStream("/sprites/Player/down0.png"));
            ...
            left0 = ImageIO.read(getClass().getResourceAsStream("/sprites/Player/left0.png"));
            ...
            right0 = ImageIO.read(getClass().getResourceAsStream("/sprites/Player/right0.png"));
            // attack sprites
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        if (isMoving) {
            ...
            if (collisionOn == false) {
                switch (direction) {
                    ...
                    case "down":
                        screenY += speed;
                        if (screenY >= (Consts.MAX_SCREEN_ROW - 1) * Consts.TILE_SIZE
                                && worldY < Consts.WORLD_SCREENS_HEIGHT) {
                    ...
                    case "right":
                        screenX += speed;
                        if (screenX >= (Consts.MAX_SCREEN_COL - 1) * Consts.TILE_SIZE
                                && worldX < Consts.WORLD_SCREENS_WIDTH) {
                ...
                default:
                    if (!((SuperDropedItem) object).armed) {
                        ...
                        if (((SuperDropedItem) object).quantity != 1) {
                            System.out.println(
                                    "You picked up " + ((SuperDropedItem) object).quantity + " " + object.name + "s");
                        } else {
    ...
    public void doItem() {
        ...
        if (actionWait > 20 && keyH.enterPressed) {
            this.gp.obj.add(new OBJ_Bomb((int) screenX / Consts.TILE_SIZE + worldX * Consts.MAX_SCREEN_COL,
                    (int) screenY / Consts.TILE_SIZE + worldY * Consts.MAX_SCREEN_ROW, true));
            // if (useItem()!=null)
            {
